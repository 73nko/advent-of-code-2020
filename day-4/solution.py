...
def check_field(dictionary, field_name):
    ...
    if field_name == 'ecl':
        if dictionary.get('ecl') in eyecolors:
            return True
        else:
            return False

    if field_name == 'hcl':
        if re.fullmatch(hcl_regex, dictionary.get('hcl')) != None:
            return True
        else:
            return False
        ...
        if re.fullmatch(hgt_regex, dictionary.get('hgt')) != None:
            if dictionary.get('hgt').endswith('cm'):
                if int((dictionary.get('hgt')).rstrip('cm')) >= 150 and \
                    ...
                    int((dictionary.get('hgt')).rstrip('in')) <= 76:
                return True
            else:
                return False
...
for entries in passport_dictionary:
    ...
    for necessary_key in keys:
        if not (necessary_key in entries):
            flag_present = False
    if flag_present:
...
print(count_fieldspresent, 'with all fields')
print(count_validvalues, 'valid data')
